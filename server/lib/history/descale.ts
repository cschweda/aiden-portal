 ...
 */
export function descaleStatus(marker: DescaleMarker | null, device: Device, thresholds: DescaleThresholds, records: readonly BrewRecord[], now: number = Date.now()): DescaleStatus {
  ...
  const brewsSince = cycles === null ? null : Math.max(0, cycles - (marker?.brews ?? 0))
  const litresSince = waterMl === null ? null : Math.max(0, waterMl - (marker?.waterMl ?? 0)) / 1000
