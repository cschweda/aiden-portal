...
| Cleaning cycles | `history/tracker.ts` | During a descale the brewer reports `cleaning: true`, `brewing: true`, `state: null`, and reuses `brewStartTime` and `brewingWaterVolumeMl` (1500 mL) for the cycle. The tracker logs the cycle to `cleanings.jsonl`, tracks no brew while it runs, and attributes any movement of the counters across it to the cycle (`cyclesDelta`, `waterDeltaMl`), which is how we will learn whether the Aiden counts a descale as a brew. |
