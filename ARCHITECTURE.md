...
| Brew time estimate | `history/estimate.ts` | The recipe fallback assumes a pour of 3.5 mL/s and a 60 s drip finish, calibrated on one watched batch brew; measured durations replace it per profile as the log fills. |
| Live water temperature | `history/tracker.ts` | VERIFIED ABSENT: `brewingWaterTemperatureC` stayed null through a whole observed brew, with the heater on, as well as while idle and during a descale. This brewer does not report it, so the trace draws a phase and heater/pump timeline instead of a temperature line. |
| `brewStartTime` during a brew | `history/tracker.ts` | Believed for up to six hours back while the brewer says it is brewing, so a restart mid-brew keeps the real start and a trusted duration. Observed: it is the current brew's start and holds until the next brew, unlike `brewEndTime`. |
| Phase code `pr` | `device.ts` | A real brew reported `state.value` as `b` for the first minute and then `pr` for the rest; `pr` is in neither reference project. It is not translated: an unknown code shows as `brewing (pr)`. |
| Nested state flags | `device.ts` | During a brew, `state` carries `lid_closed`, `missing_water`, `carafe_present`, `shower_head_present` and both basket flags. `shower_head_present` was `true` there while the top-level `showerHeadPresent` said `false`, so the nested copy is the live one and the top-level field is not to be trusted. Only the nested `missing_water` is read today. |
| Cleaning cycles | `history/tracker.ts` | Observed on 2026-09-12: a descale program is three phases (one `cleaning`, two `rinsing`, 8 to 14 minutes each, a few minutes' pause between), the brewer reports `brewing: true` with `state: null` throughout, reuses `brewStartTime` and `brewingWaterVolumeMl` (1500 mL) per phase, and counts each phase as one brew and 1.5 L in its totals. The tracker logs each phase to `cleanings.jsonl`, tracks no brew meanwhile, attributes the counters' movement to the phase, and the descale tally leaves that movement out. The restart baseline comes from the newest brew or cleaning record. |
